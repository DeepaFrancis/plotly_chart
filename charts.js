...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var bbdatasample=data.samples
    // 4. Create a variable that filters the samples for the object with the desired sample number.
    var filteredSample=bbdatasample.filter(bbSample=>bbSample.id==sample)
    ...
    var yticks = otu_ids.map(otu_id => 'OTU ' + otu_id).slice(0,10)
    ...
    Plotly.newPlot('bar', barData, barLayout);
      ...
      var metData = data.metadata.filter((obj) => obj.id == sample)[0];
